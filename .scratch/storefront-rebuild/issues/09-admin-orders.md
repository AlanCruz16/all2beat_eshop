...
- **Convex authenticates a beat after Clerk does, and the shell now waits for it.** The layout's server-side `auth()` check says the browser may be here; the Convex websocket authenticates separately and later, so a query fired in that gap throws `Unauthorized: not signed in` from `requireAdmin` — which is exactly what the first run of this screen did. `AdminAuthed` wraps the shell's children in `<AuthLoading>`/`<Unauthenticated>`/`<Authenticated>`, so 10 and 11 inherit the fix and must not reintroduce it by querying above the gate.
