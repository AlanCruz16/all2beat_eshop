...
- [ ] A Convex cron runs every 5 minutes
- [ ] It finds reservations where `status === "held"` and `expiresAt < now`, sets them `released`, and decrements `reserved`
- [ ] `committed` and already-`released` reservations are never touched
- [ ] Seam 1 tests confirm only held-and-expired rows are released and that committed/released rows are left untouched
